...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    Set<String> itemSetNotOnSale =
        customerStream.flatMap(customer -> customer.getWantToBuy().stream()).map(Item::getName)
            .filter(item -> !itemListOnSale.contains(item)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
         ...
         */
    List<Item> onSale =
        shopStream.flatMap(shop -> shop.getItemList().stream()).sorted(Comparator.comparingInt(Item::getPrice)).collect(Collectors.toList());
    Predicate<Customer> havingEnoughMoney =
        customer -> customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(item -> onSale
            .stream().filter(shopItem -> shopItem.getName().equals(item.getName())).map(Item::getPrice).findFirst().orElse(0)).sum();
